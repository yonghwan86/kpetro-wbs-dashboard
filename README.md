...
Vercel에는 `mock-site`의 정적 화면과 서버리스 API를 배포합니다. 데이터는 PostgreSQL, 비공개 회의 첨부파일과 구현·테스트 증적 이미지는 Vercel Blob에 저장됩니다. Flask/MariaDB 앱은 현재 Vercel 배포 대상이 아닙니다.
...
- 대시보드는 공개 읽기 화면이며 입력·회의·프로젝트·회원·코드·구현·테스트 관리 화면은 로그인과 화면별 권한을 사용합니다.
- 구현·테스트 계획 Excel 반영과 항목 삭제는 관리자만 할 수 있습니다. 일반 사용자는 본인 아이디 또는 이름으로 배정된 항목의 실제 일정과 증적만 수정할 수 있습니다.
- 구현·테스트 증적은 공개 URL이 아닌 비공개 Blob으로 저장되며, 화면 권한이 있는 로그인 사용자만 조회할 수 있습니다. QA 승인 뒤에만 PL 최종 심사를 진행할 수 있습니다.
